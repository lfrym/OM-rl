...
// verifier_evaluate_approximate_metric() works identically to
// verifier_evaluate_metric(), except it returns a double-precision floating
// point number and supports more metrics.
...
// run simulation cycle-by-cycle and write a JSON array of per-cycle states
// into the provided buffer.  returns the number of bytes written (excluding
// null terminator), or -1 if the buffer is too small.  each cycle state
// contains atom positions/types and arm positions/rotations.
// max_cycles limits how many cycles to simulate (0 = run to completion/error).

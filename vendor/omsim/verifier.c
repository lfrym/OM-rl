...
// Append a string to the buffer, respecting capacity.  Returns bytes written.
static int buf_append(char *buf, int pos, int buf_size, const char *str)
{
    int len = 0;
    while (str[len])
        len++;
...
int verifier_trace(void *verifier, int max_cycles, char *buf, int buf_size)
{
    struct verifier *v = verifier;
    if (!v->sf || !v->pf) {
        v->error.description = v->sf ? "invalid puzzle file" : "invalid solution file";
        return -1;
    }
    ...
    if (max_cycles <= 0)
        max_cycles = v->cycle_limit < 500 ? (int)v->cycle_limit : 500;

    int pos = 0;
    char tmp[128];

    // Write opening bracket
    int n = buf_append(buf, pos, buf_size, "[");
    if (n < 0) goto overflow;
    pos += n;
    ...
    for (int c = 0; c <= max_cycles; c++) {
        ...
        // Comma separator
        if (c > 0) {
            n = buf_append(buf, pos, buf_size, ",");
            if (n < 0) goto overflow;
            pos += n;
        }

        // Cycle header
        snprintf(tmp, sizeof(tmp), "{\"cycle\":%d,\"collision\":%s,",
            (int)board.cycle,
            board.collision ? "true" : "false");
        n = buf_append(buf, pos, buf_size, tmp);
        if (n < 0) goto overflow;
        pos += n;
        ...
        if (board.collision) {
            snprintf(tmp, sizeof(tmp),
                "\"collision_reason\":\"%s\","
                "\"collision_u\":%d,\"collision_v\":%d,",
                board.collision_reason ? board.collision_reason : "unknown",
                board.collision_location.u,
                board.collision_location.v);
            n = buf_append(buf, pos, buf_size, tmp);
            if (n < 0) goto overflow;
            pos += n;
        }
        ...
        // Atoms
        n = buf_append(buf, pos, buf_size, "\"atoms\":[");
        if (n < 0) goto overflow;
        pos += n;
        ...
        for (uint32_t i = 0; i < BOARD_CAPACITY(&board); ++i) {
            ...
            for (int j = 0; j < NUMBER_OF_ATOM_TYPES; ++j) {
                if (a & ATOM_OF_TYPE(j)) {
                    atom_name = name_for_atom_type(j);
                    break;
                }
            }
            if (!atom_name)
            ...
            int grabbed = (a & GRABBED) ? 1 : 0;
            snprintf(tmp, sizeof(tmp), "[%d,%d,\"%s\",%d]",
                p.u, p.v, atom_name, grabbed);
            n = buf_append(buf, pos, buf_size, tmp);
            if (n < 0) goto overflow;
            pos += n;
        }
        ...
        // Bonds — emit for directions 0,1,2 only (the reverse at d+3 is the same bond)
        n = buf_append(buf, pos, buf_size, "\"bonds\":[");
        if (n < 0) goto overflow;
        pos += n;
        ...
        for (uint32_t i = 0; i < BOARD_CAPACITY(&board); ++i) {
            ...
            struct vector p = board.grid.atoms_at_positions[i].position;
            for (int d = 0; d < 3; ++d) {
                atom mask = BOND_LOW_BITS << d;
                // Check for any real bond in this direction
                if (!(a & mask & REAL_BONDS))
                    continue;
                // Determine bond type
                const char *btype = "normal";
                if (a & ((1ULL << TRIPLEX_BOND_R) << d))
                    btype = "triplex_r";
                else if (a & ((1ULL << TRIPLEX_BOND_Y) << d))
                    btype = "triplex_y";
                else if (a & ((1ULL << TRIPLEX_BOND_K) << d))
                    btype = "triplex_k";

                struct vector dv = u_offset_for_direction(d);
                if (!first_bond) {
                    n = buf_append(buf, pos, buf_size, ",");
                    if (n < 0) goto overflow;
                    pos += n;
                ...
                first_bond = false;
                snprintf(tmp, sizeof(tmp), "[%d,%d,%d,%d,\"%s\"]",
                    p.u, p.v, p.u + dv.u, p.v + dv.v, btype);
                n = buf_append(buf, pos, buf_size, tmp);
                if (n < 0) goto overflow;
                pos += n;
            }
        ...
        // Arms
        n = buf_append(buf, pos, buf_size, "\"arms\":[");
        if (n < 0) goto overflow;
        pos += n;
        ...
        for (uint32_t i = 0; i < solution.number_of_arms; ++i) {
            ...
            struct mechanism *arm = &solution.arms[i];
            int grabbing = (arm->type & GRABBING) ? 1 : 0;
            snprintf(tmp, sizeof(tmp), "[%d,%d,%d,%d]",
                arm->position.u, arm->position.v,
                arm->arm_rotation, grabbing);
            n = buf_append(buf, pos, buf_size, tmp);
            if (n < 0) goto overflow;
            pos += n;
        }
        ...
        // Stop if collision or complete
        if (board.collision || board.complete)
            break;
    }
...
overflow:
    destroy(&solution, &board);
    v->error.description = "trace buffer too small";
    return -1;
}
